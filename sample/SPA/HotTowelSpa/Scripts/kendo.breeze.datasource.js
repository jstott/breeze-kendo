/* Usage Example 
var manager = new breeze.EntityManager("api/ControllerName");
  ...
  $("#grid").kendoGrid({
    ...
    dataSource: new kendo.data.extensions.BreezeDataSource({
        ...
        endpoint: "Parts"        // endPoint can be a resource string or breeze query object
    }),
...
(function ($, kendo, breeze, ko) {
  ...
  function BreezeTransport(config) {
    ...
    this.autoMapToJS = config.autoMapToJS;  // Breeze entities (not projection queries) contain recursive properties (entityAspect, entityType) - eliminate those to prevent stack overflow when grid iterates over properties
    this.map = config.map;
  ...
  kendo.data.extensions.BreezeDataSource = kendo.data.DataSource.extend({
    init: function (options) {
      ...
      options.autoMapToJS = defValue(options.autoMapToJS, true);
